...
| `compute/`    | `eks/addons`      | Selectable EKS add-ons: one Operator management/deployments toggle with automatic image/chart selection, durable executor Jobs, adaptive HA capped at three and full-cluster management by default in Ravion; retained namespace bootstrap, shared observability, Karpenter with opt-in node SSM access, load balancer controller, External Secrets Operator and EBS CSI (includes `rvn-eks-addons` module definition) | Unreleased |
