...
| `compute/`    | `eks`             | Composite EKS stack: cluster, system node group, CoreDNS, and optional Fargate profiles, provisioned in dependency order via the AWS API only; node SSM access is opt-in (includes `rvn-eks-cluster` module definition) | Unreleased |
| `compute/`    | `eks/addons`      | Selectable EKS add-ons: Operator with digest-pinned executor Jobs, adaptive HA coordinators capped at three by default, optional full-cluster management and retained namespace bootstrap; shared observability, Karpenter with opt-in node SSM access, load balancer controller, External Secrets Operator and EBS CSI (includes `rvn-eks-addons` module definition) | Unreleased |
