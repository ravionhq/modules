...
| `compute/`    | `eks/addons`      | Selectable EKS add-ons: Operator with digest-pinned executor Jobs, HA coordinators, optional full-cluster management and retained namespace bootstrap; shared observability, Karpenter with opt-in node SSM access, load balancer controller, External Secrets Operator and EBS CSI (includes `rvn-eks-addons` module definition) | Unreleased |
