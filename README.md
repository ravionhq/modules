...
| `compute/`    | `eks`             | Composite EKS stack: cluster, 2–4 node default capacity, CoreDNS, optional Fargate, and a private ARM SSM relay with read/deploy roles restricted to the account integration role and approved Ravion egress (includes `rvn-eks-cluster` module definition) | Unreleased |
| `compute/`    | `eks/addons`      | Selectable EKS add-ons: retained workload/observed namespaces with scoped Helm inventory Secret reads, stable shared observability with EKS service proxy access, Karpenter, load balancing, external secrets, storage, AMP, Loki, and CloudWatch; staged Operator retirement (includes `rvn-eks-addons` module definition) | Unreleased |
