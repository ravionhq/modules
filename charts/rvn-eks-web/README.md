...
| Ingress allow-list | `networkPolicy.enabled`, `networkPolicy.ingress.fromReleases`, `.fromNamespaces`, `.fromCIDRs` | Renders a NetworkPolicy admitting only the listed releases (same namespace), namespaces and CIDRs on the container port. Needs NetworkPolicy enforcement in the cluster; egress is never restricted. |
