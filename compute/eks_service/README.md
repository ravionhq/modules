...
| pod_identity_role_creation_enabled | Create an IAM role for the workload and bind it to the release's ServiceAccount through EKS Pod Identity; requires `cluster_name`, `release_namespace`, a ServiceAccount name and the `eks-pod-identity-agent` add-on on the cluster | `bool` | `false` | no |
| pod_identity_role_name | Name of the Pod Identity role; defaults to `<name>-task`, so a workload that also runs on ECS needs an explicit name | `string` | `null` | no |
...
| pod_identity_role_arn | ARN of the IAM role the pods assume through EKS Pod Identity; what other stacks pin to grant the workload access by identity. Null unless enabled |
| pod_identity_role_name | Name of the Pod Identity role. Null unless enabled |
| pod_identity_service_account_name | ServiceAccount in `release_namespace` the association binds the role to. Null unless enabled |
| pod_identity_association_id | ID of the EKS Pod Identity association. Null unless enabled |
...
- **Pod Identity, created here, not in the cluster stack.** The workload's IAM role is the EKS analogue of the ECS task role, so it lives with the workload: the same stack that owns the ECR repository and target group owns the role, its policies, and the `aws_eks_pod_identity_association` that binds it to the release's ServiceAccount. The cluster module's `pod_identity_associations` map stays for roles managed elsewhere. The trust policy names `pods.eks.amazonaws.com` pinned to this cluster and account through `aws:SourceArn` / `aws:SourceAccount`, and the default role name `<name>-task` mirrors the ECS task role; because IAM role names are unique per account, a service that also runs on ECS or on a second cluster sets `pod_identity_role_name` (for example `web-eks-prod`). The plan looks up the `eks-pod-identity-agent` add-on on the cluster and fails if it is missing, since without the agent the role applies cleanly and every AWS call in the pods fails at runtime. IRSA is not offered: it needs an IAM OIDC provider per cluster and a per-cluster trust policy, both of which Pod Identity removes.
