...
| pod_identity_role_creation_enabled | Create an IAM role for the workload and bind it to the release's ServiceAccount through EKS Pod Identity; requires `cluster_name`, `release_namespace` and a ServiceAccount name | `bool` | `false` | no |
| pod_identity_role_name | Name of the Pod Identity role; defaults to `<cluster_name>-<name>-task` | `string` | `null` | no |
...
- **Pod Identity, created here, not in the cluster stack.** The workload's IAM role is the EKS analogue of the ECS task role, so it lives with the workload: the same stack that owns the ECR repository and target group owns the role, its policies, and the `aws_eks_pod_identity_association` that binds it to the release's ServiceAccount. The cluster module's `pod_identity_associations` map stays for roles managed elsewhere. The trust policy names `pods.eks.amazonaws.com` pinned to this cluster and account through `aws:SourceArn` / `aws:SourceAccount`, and the default role name `<cluster_name>-<name>-task` keeps it unique per cluster and distinct from the ECS `<name>-task` role, so a service can run on both during a migration. IRSA is not offered: it needs an IAM OIDC provider per cluster and a per-cluster trust policy, both of which Pod Identity removes.
