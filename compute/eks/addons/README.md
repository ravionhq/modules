...
**IAM scoping.** The operator's Pod Identity role grants `secretsmanager:GetSecretValue`, `secretsmanager:DescribeSecret`, `ssm:GetParameter`, and `ssm:GetParameters`. Because workload secrets are created long after this stack applies, the default resource scope is every secret and parameter **in this account and region**. Set `eso_secret_and_parameter_arns` to narrow it to specific ARNs or prefixes — or to reach other regions and accounts, which the default deliberately excludes. The `ClusterSecretStore`s themselves carry a namespace condition: by default only the Ravion Operator's workload namespaces may reference them, and `eso_allowed_namespaces` overrides that list. The plan fails rather than create unconditioned stores when that list would be empty (Operator disabled or in full-cluster mode with no explicit namespaces), because an open store plus the account-wide read role would let any pod-creating principal read any secret. Secrets encrypted with a customer-managed KMS key additionally need that key listed in `eso_kms_key_arns`; the AWS-managed `aws/secretsmanager` and `aws/ssm` keys need no explicit grant.
...
Severity (`level`) and the pod name (`pod`) are attached as **structured metadata**, not labels: they are stored and filterable per line but not indexed, so neither splits one request's lines across several streams. There is deliberately **no pod-name label** — that would turn one stream per workload into one per replica per restart — and Alloy explicitly drops the `filename` label that `loki.source.file` adds, because the path contains the pod UID and would smuggle the same cardinality back in. The pod name in structured metadata is what lets the workload log view show one replica at a time.

**Operator deploy logs.** Ravion Operator runs each in-cluster deploy in an executor Job whose stdout is that deploy's log. Those pods live in the Operator namespace, which `logs_excluded_namespaces` keeps out of the store, so the exclusion is gated on the `operator.ravion.dev/module-deployment` label the Operator puts on executor pods: they are collected, every other pod in an excluded namespace is not, and their lines carry the module deployment id as the `ravion_module_deployment` label. One deploy is one stream, bounded by deploys rather than replicas, and the Ravion deploy page reads it back through Ravion Operator like any other workload log.
...
Before publishing chart `0.5.10`, verify the cross-repository resource-policy contract against its source directory. This checks both the typed module shape and a raw Helm override in the final `RVN_OPERATOR_EXECUTION_JOBS` JSON, where chart `0.5.9` dropped the ownership source:
...
`0.5.10`, which forwards executor resource-policy ownership to the running binary,
and enables two-replica HA and full-cluster management with Jobs. Chart `0.5.10`
must be published before this module version is released or applied; `0.5.9` is the
last verified published floor, but it does not forward `executionJobs.resourcesSource`.
...
Each coordinator requests 500m CPU/1Gi memory and limits at 2 CPU/2Gi. Each executor requests 100m CPU/128Mi memory/1Gi ephemeral storage and limits at 2 CPU/2Gi/20Gi. Nodes still need enough free resources to schedule the pods. Use `ravion_operator_execution_resources` for a complete explicit override; it emits `executionJobs.resourcesSource: local` so runtime policy cannot replace customer sizing.

That ownership marker is a chart/runtime contract, not just a stored Helm value. Operator binary self-update patches the Deployment image; it does **not** rerender the chart or its `RVN_OPERATOR_EXECUTION_JOBS` environment value. Upgrade the add-ons release through chart `0.5.10` before relying on an explicit resource-policy ownership setting, so the rendered configuration carries `resourcesSource`. Chart `0.5.9` remains the verified prior floor but drops this field while constructing the runtime JSON.
...
Values this module does not surface directly — `portForward.enabled`, `helmInventory.enabled`, `redaction.extraPatterns`, `image.repository`, coordinator resources, tolerations — go through `ravion_operator_helm_values`. Read the chart's `README.md` before enabling any of the opt-in capabilities. Prefer `ravion_operator_execution_resources` for executor sizing. A raw `ravion_operator_helm_values` override of `executionJobs.resources` must set `executionJobs.resourcesSource: local` in the same override; otherwise the runtime-managed policy owns those resources.
...
ravion_operator_helm_values = [yamlencode({
  executionJobs = {
    ...
    resources = {
      requests = { cpu = "250m", memory = "384Mi", "ephemeral-storage" = "1Gi" }
      limits   = { cpu = "2", memory = "3Gi", "ephemeral-storage" = "20Gi" }
    }
  }
})]
```
...
| ravion_operator_chart_version | Ravion Operator **chart** version (not the agent version), pinned per module release. Null resolves the latest; ignored for a filesystem chart. | `string` | `"0.5.10"` | no |
...
| ravion_operator_execution_resources | Optional complete executor requests and limits override; setting it pins resource policy locally. | `object` | `null` | no |
...
| name | Name slug for everything the stack creates: load balancers and security groups (`<name>-pub`, `<name>-priv`, `<name>-pub-nlb`, `<name>-priv-nlb`), Pod Identity roles, Loki bucket, log group, AMP alias, Karpenter queue; defaults to the cluster name. Up to 23 characters. | `string` | `null` | no |
