...
1. **Mints** the cluster's credential with the `ravion` provider (`ravion_operator_credential`). Ravion issues a client secret on the organization's shared WorkOS M2M application server-side, records it against this cluster's Beacon agent row, and returns a `client_id` + `client_secret` pair.
